...
function WriteOutput(strOut: string) {
  var div = <HTMLDivElement>document.querySelector("#divOut");
...
function Draw() {
  ...
  if (animationRunningSetInterval) context.fillStyle = 'green';
  else context.fillStyle = 'red';
  context.fill();
  context.lineWidth = 5;
  if (animationRunningSetInterval) context.strokeStyle = '#003300';
  else context.strokeStyle = '#330000';
  context.stroke();
}

function RenderTimeOut() {
  if (!animationRunningSetInterval) return;
  Draw();
  setTimeout(RenderTimeOut, 1);
}

function RenderRequestAnimFrame() {
  if (!animationRunningRequestAnimFrame) return;
  Draw();
...
function CanvasTest(requestAnimFrame:boolean) {
  ClearOutput();
  var div = <HTMLDivElement>document.querySelector("#divOut");
  ...
  div.appendChild(c);
  if (requestAnimFrame) RenderRequestAnimFrame();
  else RenderTimeOut();
}

function SelectionChange(evt: MouseEvent) {
  firstRun = true;
}
...
function GetSelection(): string {
  ...
  for (var i = 0; i < rdbs.length; i++) {
    var s = <HTMLInputElement> (rdbs.item(i));
    if (s.checked) return s.value;
  }
  return null;
}

function buttonClickListener(evt: MouseEvent) {
  if (firstRun) {
    ClearOutput();
    firstRun = false;
  }
...
function TimeToMilliseconds(dateTime: Date) : number {
  var milliseconds: number;
  var seconds: number = 1000;
  var minutes: number = 60 * seconds;
  var hours: number = 60 * minutes;
...
function IterateTest() {
  ...
  WriteOutput("");
  var elapsedTimes = new Array<number>();
  IntervalExecutionQueue.enqueueTask(function () { (<HTMLButtonElement>document.querySelector("#btnRun")).disabled = true; });
  for (var j = 0; j < 30; j++) {
  ...
  IntervalExecutionQueue.enqueueTask(function () {
    ...
    var min: number = elapsedTimes[0];
    var max: number = elapsedTimes[0];
    elapsedTimes.forEach(function (t: number) { sum += t; });
    var mean = sum / elapsedTimes.length;
    elapsedTimes.forEach(function (t: number) { sumOfSquares += t * t; });
    squareOfSum = sum * sum;
    elapsedTimes.forEach(function (t: number) { if (t < min) min = t; });
    elapsedTimes.forEach(function (t: number) { if (t > max) max = t; });
    var stdDev: number = Math.sqrt(sumOfSquares - (squareOfSum / elapsedTimes.length)) / (elapsedTimes.length - 1);
  ...
  IntervalExecutionQueue.enqueueTask(function () { (<HTMLButtonElement>document.querySelector("#btnRun")).disabled = false; });
}

function HookEventsTest(element: Element) {
  ...
  element.addEventListener("keyup", function (ev: KeyboardEvent) {
    if (inInputTest) {
      ClearOutput();
  ...
  element.addEventListener("mousewheel", function (ev: MouseEvent) {
    if (inInputTest) {
      ClearOutput();
...
// Thats what it says ...
function TryCatchDoNothing(f: () => void) {
  try { f(); }
  catch(ex) { }
}
  ...
  TryCatchDoNothing(function () {
    if (navigator.mimeTypes) {
      ...
      for (var i = 0; i < navigator.mimeTypes.length; i++) {
        var t: any = navigator.mimeTypes[i];
...
document.addEventListener("DOMContentLoaded", function(event) {
  var run = <HTMLButtonElement>document.querySelector("#btnRun");
  run.addEventListener("click", buttonClickListener)
  var iterate = <HTMLInputElement>document.querySelector("#rdbTestIterate");
  var input = <HTMLInputElement>document.querySelector("#rdbTestInput");
  var canvas = <HTMLInputElement>document.querySelector("#rdbTestCanvas");
